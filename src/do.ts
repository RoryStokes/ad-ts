...
type DoExpr<URI extends string, Context extends {}, Output> =
  | HKT<URI, Output>
  | ((context: Context) => HKT<URI, Output>);

type EqExpr<Context extends {}, Output extends Exclude<any, Function>> =
...
export class DoContextImpl<URI extends string, Context extends {}> {
  constructor(
    private readonly monad: Monad<URI>,
    readonly context: HKT<URI, Context>
  ) {}

  bind<Output, NewProp extends string>(
    prop: NewProp,
    value: DoExpr<URI, Context, Output>
  ) {
    return new DoContextImpl<URI, NewContext<Context, NewProp, Output>>(
      this.monad,
      this.monad.chain(this.context, context => {
  ...
  with<Output, NewProp extends string>(
    prop: NewProp,
    valueOrFn: EqExpr<Context, Output>
  ) {
    return new DoContextImpl<URI, NewContext<Context, NewProp, Output>>(
      this.monad,
      this.monad.map(
